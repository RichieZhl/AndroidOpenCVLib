...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public interface CvCameraViewFrame {
        ...
        void setBitmapMat(Mat mBitmapMat);
    };
    ...
    protected void deliverAndDrawFrame(CvCameraViewFrame frame) {
        ...
        if (bmpValid && mCacheBitmap != null) {
            Canvas holderCanvas = getHolder().lockCanvas();
            if (holderCanvas != null) {
                Bitmap tBmp = Bitmap.createBitmap(holderCanvas.getWidth(), holderCanvas.getHeight(), Bitmap.Config.ARGB_8888);
                Canvas tCanvas = new Canvas(tBmp);
                tCanvas.drawColor(0, android.graphics.PorterDuff.Mode.CLEAR);

                if (mScale != 0) {
                    tCanvas.drawBitmap(mCacheBitmap, new Rect(0,0,mCacheBitmap.getWidth(), mCacheBitmap.getHeight()),
                         new Rect((int)((tCanvas.getWidth() - mScale*mCacheBitmap.getWidth()) / 2),
                         (int)((tCanvas.getHeight() - mScale*mCacheBitmap.getHeight()) / 2),
                         (int)((tCanvas.getWidth() - mScale*mCacheBitmap.getWidth()) / 2 + mScale*mCacheBitmap.getWidth()),
                         (int)((tCanvas.getHeight() - mScale*mCacheBitmap.getHeight()) / 2 + mScale*mCacheBitmap.getHeight())), null);
                } else {
                    tCanvas.drawBitmap(mCacheBitmap, new Rect(0,0,mCacheBitmap.getWidth(), mCacheBitmap.getHeight()),
                         new Rect((tCanvas.getWidth() - mCacheBitmap.getWidth()) / 2,
                         (tCanvas.getHeight() - mCacheBitmap.getHeight()) / 2,
                         (tCanvas.getWidth() - mCacheBitmap.getWidth()) / 2 + mCacheBitmap.getWidth(),
                ...
                if (mFpsMeter != null) {
                    mFpsMeter.measure();
                    mFpsMeter.draw(holderCanvas, 20, 30);
                }
                getHolder().unlockCanvasAndPost(holderCanvas);
